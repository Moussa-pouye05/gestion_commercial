...
$prix_vente = trim($_POST['prix_vente_edit'] ?? "");
$prix_achat = trim($_POST['prix_achat_edit'] ?? "");
$quantite = trim($_POST['quantite_edit'] ?? "");
$catId = (int) ($_POST['categorie_edit'] ?? 0);
$codeBarre = trim($_POST['code_edit'] ?? "");
...
if ($nom === "") {
    ...
    $profile = $old_profile;
if (isset($_FILES["profile_picture_edit"]) && $_FILES["profile_picture_edit"]["error"] === 0) {
    ...
    $tmpName = $_FILES["profile_picture_edit"]["tmp_name"];
    $originalName = $_FILES["profile_picture_edit"]["name"];
    $size = $_FILES["profile_picture_edit"]["size"];
    ...
    $reponse = $con->updateProduit($produit);
